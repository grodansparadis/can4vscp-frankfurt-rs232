...
project. The process of updating the firmware of the module is a simple one. 
...
file](http://www.grodansparadis.com/downloads/Serial_Bootloader_AN1310_v1.05r.exe) (only available for Windows).
...
bootloader baudrate if you have problem finding the module or problems with the loading of the code. For most uses 115200 should work fine. The Frankfurt RS-232 bootloader have an autobaudrate detector so any baudrate will do. Note however that the application baudrate is locked
...
module did not enter the bootloader firmware it wil show here. Change settings and try again.
...
protection. Run the verify under the /program/verify menu to make sure the write of the firmware was OK.

Thats it!

![](./images/bootloader11.png)

Now select *run application firmware* and restart the board again and the firmware will run.
