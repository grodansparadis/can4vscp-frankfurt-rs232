...
if(lm75_read_temp_lock == 0)
 {
    ...
    lm75_temp=i2c_receive(I2C_CONTINUE);
    i2c.byte[0]=i2c_receive(I2C_QUIT);
    i2c_stop();
...
signed int lm75_get_reg(unsigned char reg_number)
{
    ...
    i2c_transmit(reg_number);
    I2C_START_RX(address);

    if(reg_number != 1)
      {
      	i2c.byte[1]=i2c_receive(I2C_CONTINUE);
        i2c.byte[0]=i2c_receive(I2C_QUIT);
      }
    else{ i2c.byte[0]=i2c_receive(I2C_QUIT); i2c.byte[1]=0; }
...
void lm75_set_tos(signed char tos_temp)
{
    ...
    I2C_START_TX(address);
    i2c_transmit(LM75_TOS_REG);


    i2c_transmit(tos_temp);       
    i2c_transmit(0);
...
void lm75_set_hyst(signed char hyst_temp)
{
    ...
    I2C_START_TX(address);
    i2c_transmit(LM75_HYST_REG);

    i2c_transmit(hyst_temp);       
    i2c_transmit(0);
...
void lm75_configure(unsigned char config_byte)
{
    ...
    I2C_START_TX(address);
    i2c_transmit(LM75_CONFIG_REG);

    i2c_transmit(config_byte);       
